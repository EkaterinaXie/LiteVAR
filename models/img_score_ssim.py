...
# MODEL_DEPTH = 30
# PIC_NUM = 250
# MODE = 2 # 0:原图 1:仅严格mask 2:cfg的全局reuse
# assert MODE in {1, 2}
# 创建解析器
...
for i in range(PIC_NUM):
    ...
    # Calculate Image Reward between corresponding images in img1_split and img2_split using SSIM
    for img1, img2 in zip(img1_split, img2_split):
        ssim_score = ssim(img1, img2, data_range=1.0).item()
...
print(f'sum ssim of all pics: {score_one_group}')
# 将列表中的每个值都除以给定的数
score_one_group_mean = [num / PIC_NUM for num in score_one_group]

# print(f'mean ssim of all pictures: {score_one_group_mean}')
for index in range(len(score_one_group_mean)):
    ...
    print(f"mean SSIM of the {index} place: {mean_each_place:.4f}")
