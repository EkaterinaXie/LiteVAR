...
mini_threshold_4 = 0.1
mini_threshold_5 = 0.08
mini_threshold_6 = 0.05
    ...
    if layer in range(3,10):
        ...
        if layer == 3:
            ...
            for i in range(y_mean1.shape[0]):
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y2[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_7: # 目前选为0.2
        ...
        if layer == 6:
            ...
            for i in range(y_mean1.shape[0]):
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y1[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_4: # 目前选为0.1
                    print(f'head = {i} mask 1 is skip')
                    print(f'NO bizhi = {y1[:,i,:,:].sum()/attention_map[:,i,:,:].sum()}')
                    new_attn1 = torch.zeros_like(y1[:,i,:,:])
                    cal_rate_1.append(int(0))
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y2[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_4: # 目前选为0.1
                    print(f'head = {i} mask 2 is skip')
                    print(f'NO bizhi = {attention_map[:, i, :, left_edge2:right_edge2].sum()/attention_map[:,i,:,:].sum()}')
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y3[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_4: # 目前选为0.1
                    print(f'head = {i} mask 3 is skip')
                    print(f'NO bizhi = {attention_map[:, i, :, left_edge3:right_edge3].sum()/attention_map[:,i,:,:].sum()}')
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y4[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_4: # 目前选为0.1
        ...
        if layer == 7: # 5 个mask
            ...
            for i in range(y_mean1.shape[0]):
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y1[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_5: # 目前选为
                    print(f'head = {i} mask 1 is skip')
                    print(f'NO bizhi = {y1[:,i,:,:].sum()/attention_map[:,i,:,:].sum()}')
                    new_attn1 = torch.zeros_like(y1[:,i,:,:])
                    cal_rate_1.append(int(0))
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y2[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_5: # 目前选为
                    print(f'head = {i} mask 2 is skip')
                    print(f'NO bizhi = {attention_map[:, i, :, left_edge2:right_edge2].sum()/attention_map[:,i,:,:].sum()}')
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y3[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_5: # 目前选为0.1
                    print(f'head = {i} mask 3 is skip')
                    print(f'NO bizhi = {attention_map[:, i, :, left_edge3:right_edge3].sum()/attention_map[:,i,:,:].sum()}')
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y4[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_5: # 目前选为
                    print(f'head = {i} mask 4 is skip')
                    print(f'NO bizhi = {attention_map[:, i, :, left_edge4:right_edge4].sum()/attention_map[:,i,:,:].sum()}')
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y5[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_5: # 目前选为
        ...
        if layer == 8:    # 六个mask
            ...
            for i in range(y_mean1.shape[0]):
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y1[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_6: # 目前选为
                    print(f'head = {i} mask 1 is skip')
                    print(f'NO bizhi = {y1[:,i,:,:].sum()/attention_map[:,i,:,:].sum()}')
                    new_attn1 = torch.zeros_like(y1[:,i,:,:])
                    cal_rate_1.append(int(0))
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y2[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_6: # 目前选为
                    print(f'head = {i} mask 2 is skip')
                    print(f'NO bizhi = {attention_map[:, i, :, left_edge2:right_edge2].sum()/attention_map[:,i,:,:].sum()}')
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y3[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_6: # 目前选为0.1
                    print(f'head = {i} mask 3 is skip')
                    print(f'NO bizhi = {attention_map[:, i, :, left_edge3:right_edge3].sum()/attention_map[:,i,:,:].sum()}')
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y4[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_6: # 目前选为0.1
                    print(f'head = {i} mask 4 is skip')
                    print(f'NO bizhi = {attention_map[:, i, :, left_edge4:right_edge4].sum()/attention_map[:,i,:,:].sum()}')
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y5[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_6: # 目前选为
                    print(f'head = {i} mask 5 is skip')
                    print(f'NO bizhi = {attention_map[:, i, :, left_edge5:right_edge5].sum()/attention_map[:,i,:,:].sum()}')
                ...
                '''排除一个mask矩形内的token值占总比很小的情况(即attnmap某一区域很黑的情况)'''
                if y6[:,i,:,:].sum()/attention_map[:,i,:,:].sum() < mini_threshold_6: # 目前选为
